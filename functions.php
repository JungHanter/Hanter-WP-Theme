...
//OLD STUFF BELOW
	...
	//custom
	include_once( get_template_directory() . '/lib/nav-menu-dropdown.php' );

	function get_mobile_menu($default_menu_name) {
		wp_nav_menu( array(
			'menu' 			 => 'primary_mobile',
			'container' 	 => 'div',
			...
			'items_wrap'     => '<select onchange="if (this.value) window.location.href=this.value">'
								. '<option value>' . $default_menu_name . '</option>'
								. '%3$s</select>',
		) );
	}

	function get_menu($default_menu_name = ' - MENU - ') {
		wp_nav_menu( array(
			'menu' => 'primary',
			'container' 	 => 'div',
			'container_class' => 'pc-menu',
		) );
		get_mobile_menu($default_menu_name);
	}

	add_action( 'genesis_before_header', 'get_mobile_menu' );
	add_action( 'genesis_before_header', 'get_menu' );
